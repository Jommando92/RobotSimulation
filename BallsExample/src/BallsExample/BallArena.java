...
public class BallArena {
  ...
  public void drawArena(MyCanvas mc) {
    for (Ball b : allBalls) b.drawBall(mc); // draw all balls
  }
  ...
  public void setPaddle(double x, double y) {
    for (Ball b : allBalls) if (b instanceof PaddleBall) b.setXY(x, y);
  }
  ...
  public double CheckBallAngle(
    ...
    for (Ball b : allBalls) if (
      b.getID() != notID && b.hitting(x, y, rad)
  ...
  public boolean checkHit(Ball target) {
    boolean ans = false;
    for (Ball b : allBalls) if (
      b instanceof GameBall && b.hitting(target)
    ) ans = true;
